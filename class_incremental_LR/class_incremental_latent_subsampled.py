...
def train_latent(x_data, y_data, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, f, csv, depth, latent_data_x, latent_data_y, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        csv.write("%d, %d, %d, %f, %i, %.3f, %.3f\n"%(c, depth, 19*latent_batches, lr, e+1, pretrain_acc, newclass_acc))
...
for cl in range(20):
    if (cl != c):
        ...
        latent_data_x, latent_data_y = generate_latent_subsample(x_batches_continual_pain, y_batches_continual_pain, latent_depth, latent_batches)
        
        for l in range(latent_batches):
            latent_data_x_list.append(latent_data_x[:,l,:,:].squeeze())
            latent_data_y_list.append(latent_data_y[l,:].squeeze())

f = open("latent_class_subsampled_%d_depth_%d.txt" % (c, latent_depth), "w")
csv = open("latent_class_incremental_subsampled_csv.csv", "a")
