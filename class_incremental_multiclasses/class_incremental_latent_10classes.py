...
parser.add_argument('--nb_epochs', type=int, default=20)       #Number of pre-training epochs
...
parser.add_argument('--latent_batches', type=int, default=1)   #Number of activation batches used as latent memory
...
def train_latent(x_data, y_data, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, x_batches_global_test, y_batches_global_test, f, csv, depth, latent_data_x, latent_data_y, c, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        new_class_acc = compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test)
        global_acc = compute_classification_accuracy(x_batches_global_test,y_batches_global_test)
 
        print("\nEpoch %i: loss=%.5f\n"%(e+1,mean_loss))
        print("\nEpoch %i: Post-Latent accuracy on the previous classes: %.3f\n"%(e+1, pretrain_acc))
        print("\nEpoch %i: Post-Latent accuracy on the removed class %d: %.3f\n" % (e+1, c, new_class_acc))
        print("\nEpoch %i: Post-Latent accuracy on all testset: %.3f\n" % (e+1, global_acc))
        f.write("\nEpoch %i for class %d: Post-Latent accuracies: %.3f %.3f %.3f\n"%(e+1, c, pretrain_acc, new_class_acc, global_acc))
        csv.write("%d, %d, %d, %i, %.3f, %.3f, %.3f\n"%(depth, latent_batches*c, c, e+1, pretrain_acc, new_class_acc, global_acc))
...
f = open("../statedicts_class_incremental/pretrain_x_10classes.pkl", "rb")
x_batches_pretrain = pickle.load(f)
f.close()

f = open("../statedicts_class_incremental/pretrain_y_10classes.pkl", "rb")
y_batches_pretrain = pickle.load(f)
f.close()

f = open("../statedicts_class_incremental/pretrain_test_x_10classes.pkl", "rb")
x_batches_pretrain_test = pickle.load(f)
f.close()

f = open("../statedicts_class_incremental/pretrain_test_y_10classes.pkl", "rb")
y_batches_pretrain_test = pickle.load(f)
f.close()

#Initializing the batch lists for the global test set
x_batches_global_test = []
y_batches_global_test = []


#Cycle classes to add a batch for each class in the global test set
for c in range(10):
    ...
    x_batches_global_test.append(x_batches_continual[latent_batches+1])
    y_batches_global_test.append(y_batches_continual[latent_batches+1])
...
for c in range(10, 20):
    ...
    x_batches_global_test.append(x_batches_continual[0])
    y_batches_global_test.append(y_batches_continual[0])




#Load the state dict
f = open("../statedicts_class_incremental/state_dict_10classes.pkl", "rb")
...
#Test pretrained net before CL training
print("\nPre-Training accuracy on the 10 classes: %.3f\n"%(compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)))
print("\nPre-Training accuracy on ALL the classes: %.3f\n"%(compute_classification_accuracy(x_batches_global_test,y_batches_global_test)))

#Generate the latent activation values
latent_data_x_list = []
latent_data_y_list = []

print("\nGenrating latent replay values from the old classes...\n")
for c in range(10):
    f = open("../statedicts_class_incremental/removed_x_class_%d.pkl" % c, "rb")
    x_batches_continual = pickle.load(f)
    f.close()
    ...
    latent_data_x, latent_data_y = generate_latent(x_batches_pretrain, y_batches_pretrain, latent_depth, latent_batches)
...
for c in range(10, 20):
    ...
    f = open("latent_10classes_depth%d_batches%d.txt" % (latent_depth, latent_batches), "a")
    csv = open("latent_10classes_incremental_csv_depth%d_batches%d_fixed_2.csv" % (latent_depth, latent_batches), "a")
    ...
    #Train function
    loss_hist = train_latent(x_batches_continual_train, y_batches_continual_train, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, x_batches_global_test, y_batches_global_test, f, csv, latent_depth, latent_data_x_list, latent_data_y_list, c, lr=2e-4, nb_epochs=nb_epochs)
    
    #Printing post-training accuracies
    pretrain_acc = compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)
    new_class_acc = compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test)
    global_acc = compute_classification_accuracy(x_batches_global_test,y_batches_global_test)
    
    print("\nPost-Training accuracy on the previous classes: %.3f\n"%(pretrain_acc))
    print("\nPost-Training accuracy on ALL the classes: %.3f\n"%(global_acc))
    print("\nPost-Training accuracy on removed class %d: %.3f\n"%(c, new_class_acc))
    f.write("\nPost-Training accuracy on the 10 classes: %.3f\n"%(pretrain_acc))
    f.write("\nPost-Training accuracy on ALL the classes: %.3f\n"%(global_acc))
    f.write("\nPost-Training accuracy on removed class %d: %.3f\n"%(c, new_class_acc))
    
    #Generate the latent activation values for the new class
    print("\nGenrating latent replay values for class %d...\n"%c)
    latent_data_x_continual, latent_data_y_continual = generate_latent(x_batches_continual, y_batches_continual, latent_depth, latent_batches)
    latent_data_x_list.append(latent_data_x_continual[:,0,:,:].squeeze())
    latent_data_y_list.append(latent_data_y_continual[0,:].squeeze())
    
    #Add new class to pretrain test
    x_batches_pretrain_test.extend(x_batches_continual_test)
    y_batches_pretrain_test.extend(y_batches_continual_test)
