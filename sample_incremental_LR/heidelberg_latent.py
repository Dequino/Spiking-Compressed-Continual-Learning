...
def train_latent(x_data, y_data, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, f, csv, depth, latent_data_x, latent_data_y, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        f.write("\nEpoch %i: Post-Latent accuracy on the 11 speakers: %.3f\n"%(e+1, compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)))
        f.write("\nEpoch %i: Post-Latent accuracy on the removed speaker %d: %.3f\n" % (e+1, speaker, (compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test))))
        csv.write("%d, %d, %d, %i, %.3f\n"%(speaker, depth, latent_batches, e+1, compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)))
        csv.write("%d, %d, %d, %i, %.3f\n" % (speaker, depth, latent_batches, e+1, (compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test))))
...
f = open("latent_speaker_%d_depth_%d.txt" % (speaker, latent_depth), "w")
csv = open("latent_csv.csv", "a")
...
loss_hist = train_latent(x_batches_continual_train, y_batches_continual_train, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, f, csv, latent_depth, latent_data_x_list, latent_data_y_list, lr=2e-4, nb_epochs=nb_epochs)
