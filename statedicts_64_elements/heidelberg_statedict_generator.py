...
def train(x_data, y_data, lr=1e-3, nb_epochs=10):
    ...
    f = open("state_dict_speaker_%d.pkl" % speaker, "wb")
...
#Separating the dataset based on the selected speaker. "pretrain" is the dataset containing all samples by every speaker EXCEPT the one selected in args, whereas the "continual" dataset is the one containing the samples of the selcted speaker.
x_batches_pretrain, y_batches_pretrain, speaker_batches_pretrain = speaker_sparse_data_generator_from_hdf5_spikes(x_train, y_train, batch_size, nb_steps, nb_inputs, max_time, speakers=speakers_train, speaker=speaker, train=True)
x_batches_continual, y_batches_continual, speaker_batches_continual = speaker_sparse_data_generator_from_hdf5_spikes(x_train, y_train, batch_size, nb_steps, nb_inputs, max_time, speakers=speakers_train, speaker=speaker, train=False)
#Debug code: counts how many times each speaker (except the one removed) contributed in the pre-train set
labels_count = [None] * 12
for batch in speaker_batches_pretrain:
    for s in batch:
        if labels_count[s] is None:
            labels_count[s] = 1
        else:
            labels_count[s] += 1

print("\nSpeaker count in pretrain for speaker %d:\n" % speaker)
print(labels_count)
...
pretrain_x_file = open("pretrain_x_speaker_%d.pkl" % speaker, "wb")
pretrain_y_file = open("pretrain_y_speaker_%d.pkl" % speaker, "wb")
pretrain_test_x_file = open("pretrain_test_x_speaker_%d.pkl" % speaker, "wb")
pretrain_test_y_file = open("pretrain_test_y_speaker_%d.pkl" % speaker, "wb")
...
removed_x_file = open("removed_x_speaker_%d.pkl" % speaker, "wb")
removed_y_file = open("removed_y_speaker_%d.pkl" % speaker, "wb")
