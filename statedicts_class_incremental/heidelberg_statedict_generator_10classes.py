...
#           labels_filtered (list): list of int64 containing the labels of the non-filtered speakers samples
#           speakers_filtered (list): list of uint64 containing the speaker ID of the non-filtered speakers samples
def filter_classes(firing_times, units_fired, labels, speakers):
    ft_filtered = []
    uf_filtered = []
    labels_filtered = []
    speakers_filtered = []
...
#Twin function of the filter_speaker function described above.
#The arguments and return values are of the same type described above. The only difference being that this function filters all sample BUT the ones of the selected speaker.
def filter_class_inverse(firing_times, units_fired, labels, c, speakers):
    ft_filtered = []
    uf_filtered = []
    labels_filtered = []
    speakers_filtered = []
...
def class_sparse_data_generator_from_hdf5_spikes(X, y, batch_size, nb_steps, nb_units, max_time, speakers, shuffle=True, c=0, train=True):
    ...
    if train:
        firing_times, units_fired, labels_, speakers_filtered = filter_classes(firing_times, units_fired, labels_, speakers)
    else:
        firing_times, units_fired, labels_, speakers_filtered = filter_class_inverse(firing_times, units_fired, labels_, c, speakers)
...
def train(x_data, y_data, lr=1e-3, nb_epochs=10):
    ...
    f = open("state_dict_10classes.pkl", "wb")
...
x_batches_pretrain, y_batches_pretrain, speaker_batches_pretrain = class_sparse_data_generator_from_hdf5_spikes(x_train, y_train, batch_size, nb_steps, nb_inputs, max_time, speakers=speakers_train, c=0, train=True)
...
for c in range(10, 20):
    ...
    for batch in y_batches_continual:
        for l in batch:
...
print("\nPre-Training accuracy on the other 10 classes: %.3f\n"%(compute_classification_accuracy(x_batches_continual,y_batches_continual)))
print("\nPre-Training accuracy on the entire testset: %.3f\n"%(compute_classification_accuracy(x_batches_pretrain_test+x_batches_continual,y_batches_pretrain_test+y_batches_continual)))


pretrain_x_file = open("pretrain_x_10classes.pkl", "wb")
pretrain_y_file = open("pretrain_y_10classes.pkl", "wb")
pretrain_test_x_file = open("pretrain_test_x_10classes.pkl", "wb")
pretrain_test_y_file = open("pretrain_test_y_10classes.pkl", "wb")
