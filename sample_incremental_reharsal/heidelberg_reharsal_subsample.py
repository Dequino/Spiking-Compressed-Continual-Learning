...
parser.add_argument('--latent_subsample', type=int, default=20) #Number of subsampled timesteps
...
def train_reharsal(x_data, y_data, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, f, csv, latent_data_x, latent_data_y, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        for x_local, y_local in zip(x_data, y_data):
            ...
            y_local.to(device)
            #Run the SNN here
            output,recs = run_snn(x_local.to_dense(), True, 1)
        ...
        for latent_local_x, latent_local_y in zip(latent_data_x, latent_data_y):
            ...
            #Run the SNN here
            output,recs = run_snn(latent_local_x, True, nb_steps//latent_subsample)
        ...
        f.write("\nEpoch %i: Post-Latent accuracy on the 11 speakers: %.3f\n"%(e+1, pretrain_acc))
        f.write("\nEpoch %i: Post-Latent accuracy on the removed speaker %d: %.3f\n" % (e+1, speaker, (continual_acc)))
        csv.write("%d, %d, %i, %.3f, %.3f\n"%(speaker, latent_batches, e+1, pretrain_acc, continual_acc))
...
def compute_classification_accuracy(x_data, y_data):
    ...
    for x_local, y_local in zip(x_data, y_data):
        x_local.to(device)
        y_local.to(device)
        output,_ = run_snn(x_local.to_dense(), False, 1)
...
def generate_latent_subsample(x_batches_pretrain, y_batches_pretrain, latent_batches):
    ...
    for n, (x, y) in enumerate(zip(x_batches_pretrain, y_batches_pretrain)):
        x.to(device)
        y.to(device)
        latent_activations_rec.append(x.to_dense())
...
latent_data_x, latent_data_y = generate_latent_subsample(x_batches_pretrain, y_batches_pretrain, latent_batches)
...
f = open("reharsal_speaker_%d.txt" % (speaker), "w")
csv = open("reharsal_csv.csv", "a")
...
loss_hist = train_reharsal(x_batches_continual_train, y_batches_continual_train, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, f, csv, latent_data_x_list, latent_data_y_list, lr=1e-3, nb_epochs=nb_epochs)
