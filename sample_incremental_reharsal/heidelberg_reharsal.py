...
#w4 = torch.empty((nb_hidden_3, nb_hidden_4),  device=device, dtype=dtype, requires_grad=True)
#torch.nn.init.normal_(w4, mean=0.0, std=weight_scale/np.sqrt(nb_hidden_3))
...
#v4 = torch.empty((nb_hidden_4, nb_hidden_4), device=device, dtype=dtype, requires_grad=True)
#torch.nn.init.normal_(v4, mean=0.0, std=weight_scale/np.sqrt(nb_hidden_4))
...
#           speaker (int): speaker filtered, selected by argument on command line
#           shuffle (bool): if True, the dataset batches are randomly reordered
#           train (bool): if True, filter_speaker is called, if False, filter_speaker_inverse is called instead
#Returns:
...
def speaker_sparse_data_generator_from_hdf5_spikes(X, y, batch_size, nb_steps, nb_units, max_time, speakers, speaker, shuffle=True, train=True):
    ...
    if train:
        firing_times, units_fired, labels_, speakers_filtered = filter_speaker(firing_times, units_fired, labels_, speakers, speaker)
    else:
        firing_times, units_fired, labels_, speakers_filtered = filter_speaker_inverse(firing_times, units_fired, labels_, speakers, speaker)
...
def train_reharsal(x_data, y_data, x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_test, y_batches_continual_test, f, csv, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        print("\nEpoch %i: loss=%.5f\n"%(e+1,mean_loss))
        print("\nEpoch %i: Post-Reharsal accuracy on the 11 speakers: %.3f\n"%(e+1, compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)))
        print("\nEpoch %i: Post-Reharsal accuracy on the removed speaker %d: %.3f\n" % (e+1, speaker, (compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test))))
        f.write("\nEpoch %i: Post-Reharsal accuracy on the 11 speakers: %.3f\n"%(e+1, compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)))
        f.write("\nEpoch %i: Post-Reharsal accuracy on the removed speaker %d: %.3f\n" % (e+1, speaker, (compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test))))
        csv.write("%d, %d, %i, %.3f\n"%(speaker, reharsal_size, e+1, compute_classification_accuracy(x_batches_pretrain_test,y_batches_pretrain_test)))
        csv.write("%d, %d, %i, %.3f\n" % (speaker, reharsal_size, e+1, (compute_classification_accuracy(x_batches_continual_test,y_batches_continual_test))))
...
for j in range(reharsal_size):
    x_batches_continual_train.append(x_batches_pretrain[j])
    y_batches_continual_train.append(y_batches_pretrain[j])
...
f = open("reharsal_speaker_%d_batches_%d.txt" % (speaker, reharsal_size), "w")
csv = open("rehearsal_64_csv.csv", "a")
...
loss_hist = train_reharsal(x_batches_continual_train, y_batches_continual_train, x_batches_pretrain_test,y_batches_pretrain_test, x_batches_continual_test,y_batches_continual_test, f, csv, lr=2e-4, nb_epochs=50)
