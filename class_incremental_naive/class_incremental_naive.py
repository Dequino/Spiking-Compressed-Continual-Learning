...
def naive_class_incremental_train(x_data_pretrain, y_data_pretrain, x_data_continual, y_data_continual, x_data_test, y_data_test, file, csv, lr=1e-3, nb_epochs=10):
    ...
    for e in range(nb_epochs):
        ...
        csv.write("%d, %f, %i, %.3f, %.3f\n"%(c, lr, e+1, pretrain_acc, newclass_acc))     
...
f = open("class_%d_naive_class_inc_train.txt" % c, 'w')
csv = open("naive_class_incremental_csv.csv", "a")
...
loss_hist = naive_class_incremental_train(x_batches_pretrain_test, y_batches_pretrain_test, x_batches_continual_train, y_batches_continual_train, x_batches_continual_test, y_batches_continual_test, file=f, csv=csv, lr=2e-4, nb_epochs=nb_epochs)
